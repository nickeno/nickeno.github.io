(function () {
    ...
    function initCopyButtons() {
        ...
        document.querySelectorAll('[data-copy-license]').forEach((button) => {
            button.addEventListener('click', async () => {
                ...
                clone.querySelectorAll('[data-copy-ignore]').forEach((item) => item.remove());
    ...
    function initScrollAnimations() {
        if (reduceMotion) return;
        if (typeof gsap === 'undefined' || typeof ScrollTrigger === 'undefined') {
            root.classList.add('no-gsap-motion');
            return;
        }
        ...
        const pricingHeading = document.querySelector('.pricing-section .section-heading h2:not(.package-heading)');
        if (pricingHeading) {
